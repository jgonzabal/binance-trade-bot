...
class BinanceAPIManager:
    ...
    def _adjust_bnb_balance(self, origin_coin: Coin, target_coin: Coin):
        ...
        # Default value of AUTO_ADJUST_BNB_BALANCE_RATE is 3,
        # means trying to buy 3x BNB compare to the commision needed by the coming order.
        # Put "3x" as default since: 1. buy commision, 2. sell commision, 3.
        # buffer, since selling price may rise and then needs more comission.
        fee_amount_bnb_ceil = math.ceil(
            (fee_amount_bnb * self.config.AUTO_ADJUST_BNB_BALANCE_RATE - bnb_balance) * 10 ** alt_tick
        ) / float(10 ** alt_tick)
    ...
    def _buy_alt(
        self, origin_coin: Coin, target_coin: Coin, buy_price: float, buy_quantity: float = None
